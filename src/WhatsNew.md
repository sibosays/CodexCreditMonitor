...
- Leest het werkelijk nieuwste lokale limietsnapshot, ook wanneer een sessiebestand ouder lijkt of een grote logregel na de limietregistratie staat. Een verversverzoek tijdens een lopende uitlezing wordt daarna opnieuw uitgevoerd.
...
- Selects the genuinely newest local limit snapshot even when its session file looks older or a large log entry follows the limit record. A refresh request received during an active read is performed again afterwards.
